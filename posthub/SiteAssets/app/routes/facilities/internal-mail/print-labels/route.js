...
import { packageDetailsColumn, destinationColumn, recipientColumn } from '../../../../utils/package-table-columns.js'
import { createQrLabelCard } from '../../utils/qrLabel.js'
import { printQrLabelPdf } from '../../utils/qrLabelPdf.js'

export default defineRoute(async (config) => {
  config.setRouteTitle('Print Labels')
  ...
  const pageHeader = new Container([
    new Container([
      ...
      new Text('Print Labels', {
        type: 'h1',
        class: 'posthub__page-title'
      }),
    ], { class: 'posthub__title-with-icon' }),
    new Text('Search for a sender to print QR labels', {
      type: 'p',
  ...
  const searchPrompt = new Container([
    new Container([
      ...
      new Text('Type a name or email to find pending mail', {
        type: 'p',
        class: 'posthub__prompt-subtitle'
      }),
      new FieldLabel('Sender', senderPicker, { class: 'print-labels__sender-field' }),
  ...
  async function handleSenderSelect(email, displayName) {
    ...
    try {
      ...
      senderSelected = true
      Toast.success(`Found ${packages.length} pending mail item(s) for ${displayName}`)
      updateView()
    } catch {
  ...
  function markRowPrinted(e) {
    e.stopPropagation()
    e.target.closest('.posthub__table-row')?.classList.add('print-labels__row--printed')
  }
  ...
  function updateView() {
    if (!senderSelected) {
      ...
      tableContainer.children = [
        new Container([
          new Container([
            new Text('No pending mail found', {
              type: 'h2',
              class: 'posthub__prompt-title'
            }),
            new Text('This sender has no mail with "pending" status.', {
              type: 'p',
              class: 'posthub__prompt-subtitle'
            }),
      ...
      const table = createPackageTable({
        ...
        tableClass: 'print-labels__table',
      })
  ...
  const printContainer = new Container([], { class: 'print-labels__print-container' })
